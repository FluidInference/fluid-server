...
class OpenVINOWhisperRuntime(BaseRuntime):
    ...
    def get_executor(cls) -> ThreadPoolExecutor:
        ...
        if cls._whisper_executor is None:
            cls._whisper_executor = ThreadPoolExecutor(
                max_workers=2, 
    ...
    async def load(self) -> None:
        ...
        async with self._load_lock:
            ...
            try:
                ...
                raise ValueError("openvino_genai not available. Install with: pip install openvino-genai")
            except Exception as e:
                logger.error(f"Failed to load Whisper model: {e}")
                raise
    
    async def transcribe(self, audio_data: bytes, language: Optional[str] = None, return_timestamps: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            with wave.open(io.BytesIO(audio_data), 'rb') as wav_file:
                ...
                logger.info(f"Audio format: {channels} channel(s), {sample_rate}Hz, {sample_width} bytes per sample")
                
                # Convert to float32 array based on sample width
                if sample_width == 1:
                    # 8-bit unsigned
                    audio_float32 = np.frombuffer(frames, dtype=np.uint8).astype(np.float32) / 127.5 - 1.0
                elif sample_width == 2:
                    # 16-bit signed
                    audio_float32 = np.frombuffer(frames, dtype=np.int16).astype(np.float32) / 32768.0
                elif sample_width == 3:
                    # 24-bit (rare, treat as 32-bit)
                    # Pad 24-bit to 32-bit
                    padded_frames = np.frombuffer(frames, dtype=np.uint8)
                    padded_frames = np.pad(padded_frames.reshape(-1, 3), ((0, 0), (0, 1)), mode='constant')
                    ...
                    try:
                        ...
                        if np.max(np.abs(audio_float32)) > 1.1:
                            ...
                            audio_float32 = np.frombuffer(frames, dtype=np.int32).astype(np.float32) / 2147483648.0
                    except:
                        # Fall back to int32
                        audio_float32 = np.frombuffer(frames, dtype=np.int32).astype(np.float32) / 2147483648.0
                else:
            ...
            logger.info(f"Audio loaded: {len(audio_float32)} samples at {sample_rate}Hz, duration: {len(audio_float32)/sample_rate:.2f}s")
