...
async def health_check(
    ...
    if runtime_manager.current_runtime:
        ...
        model_type = (
            "llm"
            if hasattr(runtime_manager.current_runtime, "pipeline")
            and hasattr(runtime_manager.current_runtime.pipeline, "generate")
            else "whisper"
        )
    ...
    if runtime_manager.warmup_status.get("in_progress", False):
        ...
        runtime_manager.warmup_status.get("whisper") == "ready"
        or runtime_manager.warmup_status.get("llm") == "ready"
    ):
        status = "ready"
    else:
    ...
    if runtime_manager.warmup_status.get("start_time") or runtime_manager.warmup_status.get(
        "in_progress"
    ):
        ...
        if runtime_manager.warmup_status.get("start_time") and runtime_manager.warmup_status.get(
            "in_progress"
        ):
            import time
