...
class RuntimeManager:
    ...
    def __init__(self, config: ServerConfig) -> None:
        ...
        self.llm_runtime: Optional[BaseRuntime] = None
        self.whisper_runtime: Optional[BaseRuntime] = None
        self.loaded_llm_model: Optional[str] = None
        self.loaded_whisper_model: Optional[str] = None
    ...
    async def _warm_up_models(self) -> None:
        ...
        try:
            ...
            if self.config.llm_model not in self.available_models.get("llm", []):
                logger.info(f"LLM model '{self.config.llm_model}' not found locally, downloading...")
                self.warmup_status["current_task"] = f"Downloading LLM model '{self.config.llm_model}'..."
                self.download_status[f"llm:{self.config.llm_model}"] = "downloading"
                llm_available = await self.downloader.ensure_model_available("llm", self.config.llm_model)
                if llm_available:
            ...
            if self.config.whisper_model not in self.available_models.get("whisper", []):
                logger.info(f"Whisper model '{self.config.whisper_model}' not found locally, downloading...")
                self.warmup_status["current_task"] = f"Downloading Whisper model '{self.config.whisper_model}'..."
                self.download_status[f"whisper:{self.config.whisper_model}"] = "downloading"
                whisper_available = await self.downloader.ensure_model_available("whisper", self.config.whisper_model)
                if whisper_available:
                    # Refresh model discovery after download
                    self.available_models = ModelDiscovery.find_models(self.config.model_path)
                    self.download_status.pop(f"whisper:{self.config.whisper_model}", None)
                    logger.info(f"Whisper model '{self.config.whisper_model}' downloaded successfully")
                else:
                    self.download_status[f"whisper:{self.config.whisper_model}"] = "failed"
                    self.warmup_status["whisper"] = "failed"
                    logger.warning(f"Whisper model '{self.config.whisper_model}' could not be downloaded")
            ...
            if self.config.whisper_model in self.available_models.get("whisper", []):
                try:
                    ...
                    self.warmup_status["current_task"] = f"Loading and compiling Whisper model '{self.config.whisper_model}' on NPU (this may take several minutes)..."
                    logger.info(f"Pre-loading Whisper model '{self.config.whisper_model}' for warm-up")
                    await self.load_whisper(self.config.whisper_model)
                    self.warmup_status["whisper"] = "ready"
                    logger.info(f"Whisper model '{self.config.whisper_model}' loaded and compiled successfully")
            ...
            if self.config.llm_model in self.available_models.get("llm", []):
                try:
                    ...
                    self.warmup_status["current_task"] = f"Pre-loading LLM model '{self.config.llm_model}' on GPU..."
            ...
            logger.info(f"Model warm-up completed in {elapsed:.1f}s - Whisper: {self.warmup_status['whisper']}, LLM: {self.warmup_status['llm']}")
    ...
    async def load_llm(self, model_name: str = None) -> OpenVINOLLMRuntime:
        ...
        if (self.llm_runtime is not None and 
        ...
        @retry_async(
            ...
            on_retry=lambda attempt, exc: logger.info(f"Retrying LLM model load (attempt {attempt}): {exc}")
        )
    ...
    async def load_whisper(self, model_name: str = None) -> OpenVINOWhisperRuntime:
        ...
        if (self.whisper_runtime is not None and 
        ...
        if self.whisper_runtime is not None and self.loaded_whisper_model != model_to_load:
            logger.info(f"Unloading Whisper '{self.loaded_whisper_model}' to load '{model_to_load}'")
        ...
        @retry_async(
            ...
            backoff_factor=1.5,
            exceptions=(RuntimeError, ValueError, OSError),
            on_retry=lambda attempt, exc: logger.info(f"Retrying Whisper model load (attempt {attempt}): {exc}")
        )
        async def _load_with_retry():
    ...
    async def get_llm(self, model_name: str = None) -> OpenVINOLLMRuntime:
        ...
        if (self.llm_runtime is None or 
            self.loaded_llm_model != model_to_get):
    ...
    async def get_whisper(self, model_name: str = None) -> OpenVINOWhisperRuntime:
        ...
        if (self.whisper_runtime is None or 
            self.loaded_whisper_model != model_to_get):
        ...
        async def cleanup_idle_model():
            ...
            while True:
                try:
                    ...
                    if (self.llm_runtime is not None and 
                        hasattr(self.llm_runtime, 'last_used')):
                        idle_time = current_time - self.llm_runtime.last_used
                        if idle_time >= idle_threshold:
                            logger.info(f"Unloading idle LLM model '{self.loaded_llm_model}' (idle for {idle_time/60:.1f} minutes)")
                            try:
                                await self.llm_runtime.unload()
                                self.llm_runtime = None
                                self.loaded_llm_model = None
                    ...
                    if (self.whisper_runtime is not None and 
                        hasattr(self.whisper_runtime, 'last_used')):
                        idle_time = current_time - self.whisper_runtime.last_used
                        if idle_time >= idle_threshold:
                            logger.info(f"Unloading idle Whisper model '{self.loaded_whisper_model}' (idle for {idle_time/60:.1f} minutes)")
                            try:
                                await self.whisper_runtime.unload()
                                self.whisper_runtime = None
                                self.loaded_whisper_model = None
